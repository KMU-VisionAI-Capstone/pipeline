...
from image_captioning.captioning import captioning
...
def on_click(image_input, alpha):
    ...
    for file_name, similarity in ranked_results[:5]:  # 상위 5개 결과만 표시
        results_html += f"<p><strong>{file_name}:</strong> {similarity:.4f}</p>"

    return classification_html, caption_html, results_html
...
with gr.Blocks() as demo:
    ...
    predict_button.click(
        ...
        outputs=[classification_output, caption_output, matching_results_output],
    )
